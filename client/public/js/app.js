...
const BOTS = [
  { name: 'Pawnsy',      rating: 600,  tier: 'Beginner',    icon: '\u265F', color: '#34D399' },
  { name: 'Rookie Rae',  rating: 850,  tier: 'Beginner',    icon: '\u265F', color: '#34D399' },
  { name: 'Blunderbot',  rating: 1050, tier: 'Novice',      icon: '\u265F', color: '#34D399' },
  { name: 'Casual Cal',  rating: 1250, tier: 'Casual',      icon: '\u265E', color: '#F5C518' },
  { name: 'Steady Sam',  rating: 1450, tier: 'Club',        icon: '\u265E', color: '#F5C518' },
  { name: 'Tactic Tom',  rating: 1650, tier: 'Club',        icon: '\u265E', color: '#F5C518' },
  { name: 'Sharp Sara',  rating: 1850, tier: 'Strong',      icon: '\u265D', color: '#FF7A1A' },
  { name: 'Knight Owl',  rating: 2050, tier: 'Expert',      icon: '\u265C', color: '#FF7A1A' },
  { name: 'Rookslayer',  rating: 2250, tier: 'Master',      icon: '\u265C', color: '#FF7A1A' },
  { name: 'Queen Bea',   rating: 2500, tier: 'Master',      icon: '\u265B', color: '#E11D2E' },
  { name: 'Deep Gambit', rating: 2750, tier: 'Grandmaster', icon: '\u265B', color: '#E11D2E' },
  { name: 'Overlord',    rating: 3000, tier: 'Maximum',     icon: '\u265A', color: '#E11D2E' }
];

function botPickerStyle() {
  if (document.getElementById('bp-style')) return;
  const st = document.createElement('style'); st.id = 'bp-style';
  st.textContent =
    "#bp-bg{position:fixed;inset:0;z-index:1700;display:flex;align-items:center;justify-content:center;padding:16px;background:rgba(10,12,16,.9);backdrop-filter:blur(5px);animation:bpFade .25s ease;}" +
    "@keyframes bpFade{from{opacity:0}to{opacity:1}}" +
    "#bp-card{width:min(560px,96vw);max-height:90vh;overflow-y:auto;background:#14171C;border:1px solid #2A2F37;border-radius:18px;padding:22px;box-shadow:0 24px 60px rgba(0,0,0,.6);}" +
    "#bp-head{display:flex;align-items:center;justify-content:space-between;margin-bottom:16px;}" +
    "#bp-title{font-family:'Antonio',sans-serif;font-weight:700;font-size:23px;color:#F5F1EA;letter-spacing:1px;}" +
    "#bp-x{width:34px;height:34px;border:none;border-radius:50%;background:rgba(255,255,255,.08);color:#F5F1EA;font-size:21px;cursor:pointer;}" +
    "#bp-x:hover{background:rgba(255,255,255,.16);}" +
    "#bp-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:10px;}" +
    ".bp-bot{display:flex;flex-direction:column;align-items:center;gap:3px;background:#1B1F27;border:1px solid #2A2F37;border-radius:12px;padding:14px 8px;cursor:pointer;transition:transform .15s,border-color .15s,background .15s;}" +
    ".bp-bot:hover{transform:translateY(-3px);background:#222732;border-color:#3D4452;}" +
    ".bp-emoji{font-size:30px;line-height:1;}" +
    ".bp-name{font-family:'Antonio',sans-serif;font-weight:700;font-size:15px;color:#F5F1EA;text-align:center;}" +
    ".bp-rating{font-family:'JetBrains Mono',monospace;font-weight:700;font-size:16px;}" +
    ".bp-tier{font-family:'JetBrains Mono',monospace;font-size:9px;letter-spacing:1px;color:#8B92A0;text-transform:uppercase;}" +
    "@media(max-width:520px){#bp-grid{grid-template-columns:repeat(2,1fr);}}";
  document.head.appendChild(st);
}

function openBotPicker() {
  botPickerStyle();
  if (document.getElementById('bp-bg')) return;
  const bg = document.createElement('div'); bg.id = 'bp-bg';
  bg.innerHTML = '<div id="bp-card"><div id="bp-head"><div id="bp-title">Choose your CPU opponent</div><button id="bp-x" aria-label="Close">\u00d7</button></div><div id="bp-grid">' +
    BOTS.map(function (b) {
      return '<button class="bp-bot" data-r="' + b.rating + '">' +
        '<div class="bp-emoji" style="color:' + b.color + '">' + b.icon + '</div>' +
        '<div class="bp-name">' + b.name + '</div>' +
        '<div class="bp-rating" style="color:' + b.color + '">' + b.rating + '</div>' +
        '<div class="bp-tier">' + b.tier + '</div></button>';
    }).join('') + '</div></div>';
  document.body.appendChild(bg);
  bg.querySelector('#bp-x').onclick = function () { bg.remove(); };
  bg.onclick = function (e) { if (e.target === bg) bg.remove(); };
  bg.querySelectorAll('.bp-bot').forEach(function (btn) {
    btn.onclick = function () {
      ...
      wsSend({ type: 'play_bot', rating: r });
      const ss = document.getElementById('search-status');
      if (ss) ss.innerHTML = 'Starting a game vs CPU ' + r + '<span class="dot-anim"></span>';
    };
  });
}
...
function replayStyle() {
  ...
  st.textContent =
    "#rp-bg{position:fixed;inset:0;z-index:1600;background:#0E1116;display:flex;align-items:center;justify-content:center;animation:rpFade .25s ease;}" +
    "@keyframes rpFade{from{opacity:0}to{opacity:1}}" +
    "#rp-card{width:100%;height:100%;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:18px;box-sizing:border-box;}" +
    "#rp-head{width:min(86vw,68vh);display:flex;align-items:center;justify-content:space-between;}" +
    "#rp-title{font-family:'Antonio',sans-serif;font-weight:700;font-size:22px;color:#F5F1EA;letter-spacing:1px;}" +
    "#rp-x{width:36px;height:36px;border:none;border-radius:50%;background:rgba(255,255,255,.08);color:#F5F1EA;font-size:22px;cursor:pointer;}" +
    "#rp-x:hover{background:rgba(255,255,255,.16);}" +
    "#rp-board{width:min(86vw,68vh);aspect-ratio:1/1;display:grid;grid-template-columns:repeat(8,1fr);grid-template-rows:repeat(8,1fr);border-radius:10px;overflow:hidden;box-shadow:0 0 0 3px rgba(255,122,26,.18),0 24px 60px rgba(0,0,0,.55);}" +
    ...
    ".rp-last::before{content:'';position:absolute;inset:0;background:rgba(255,122,26,0.32);}" +
    ".rp-piece{width:86%;height:86%;position:relative;z-index:1;}" +
    "#rp-status{font-family:'JetBrains Mono',monospace;font-size:12px;letter-spacing:2px;color:#C8CDD6;min-height:14px;}" +
    "#rp-ctrl{width:min(86vw,68vh);display:flex;gap:8px;}" +
    "#rp-ctrl button{flex:1;background:#191D24;border:1px solid #2A2F37;color:#F5F1EA;border-radius:9px;padding:14px 0;font-size:16px;cursor:pointer;}" +
    "#rp-ctrl button:hover{border-color:#FF7A1A;}" +
    "#rp-ctrl button.rp-play{flex:1.8;background:#FF7A1A;border-color:#FF7A1A;color:#0E1116;font-family:'Antonio',sans-serif;font-weight:700;letter-spacing:1px;}";
  document.head.appendChild(st);
}
...
function blunderModalStyle() {
  if (document.getElementById('bm-style')) return;
  const st = document.createElement('style'); st.id = 'bm-style';
  st.textContent =
    "#blunder-modal-bg{position:fixed;inset:0;z-index:1400;pointer-events:none;animation:bmFade .3s ease;transition:opacity .4s ease;}" +
    "#blunder-modal-bg.bm-out{opacity:0;}" +
    "#blunder-modal{position:fixed;right:18px;top:50%;transform:translateY(-50%);width:288px;max-width:86vw;max-height:90vh;overflow-y:auto;pointer-events:auto;background:#FFFCF5;color:#2A2118;border-radius:15px;box-shadow:0 18px 50px rgba(0,0,0,.5);font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;animation:bmInSide .4s cubic-bezier(.18,.9,.32,1.4);}" +
    "#blunder-modal .bm-pad{padding:16px 17px 12px;}" +
    "#blunder-modal .bm-x{position:absolute;top:9px;right:9px;width:27px;height:27px;border:none;border-radius:50%;background:rgba(0,0,0,.06);color:#2A2118;font-size:18px;line-height:1;cursor:pointer;display:flex;align-items:center;justify-content:center;}" +
    "#blunder-modal .bm-x:hover{background:rgba(0,0,0,.12);}" +
    "#blunder-modal .bm-head{font-family:Antonio,sans-serif;font-weight:700;font-size:20px;letter-spacing:.5px;color:#C0392B;margin:0 26px 7px 0;}" +
    "#blunder-modal .bm-body{font-size:12.5px;line-height:1.45;}#blunder-modal .bm-body strong{color:#C0392B;}" +
    "#blunder-modal .bm-sub{font-family:'JetBrains Mono',monospace;font-size:9px;letter-spacing:2.5px;text-transform:uppercase;color:#9a8f7d;margin:14px 0 4px;}" +
    "#blunder-modal .bm-move{display:flex;gap:9px;align-items:flex-start;padding:7px 0;border-top:1px solid #efe7d8;}#blunder-modal .bm-move:first-of-type{border-top:none;}" +
    "#blunder-modal .bm-badge{flex:none;width:19px;height:19px;border-radius:50%;color:#fff;font-family:Antonio,sans-serif;font-weight:700;font-size:11px;display:flex;align-items:center;justify-content:center;margin-top:1px;}" +
    "#blunder-modal .bm-san{font-family:'JetBrains Mono',monospace;font-weight:700;font-size:13px;}" +
    "#blunder-modal .bm-lead{font-size:10px;color:#9a8f7d;font-family:Antonio,sans-serif;letter-spacing:.5px;margin-left:4px;}" +
    "#blunder-modal .bm-why{font-size:11px;line-height:1.35;color:#4a4136;margin-top:2px;}" +
    "#blunder-modal .bm-upsell{margin-top:13px;background:linear-gradient(135deg,#fff6e0,#ffe9c2);border:1px solid #f5c518;border-radius:10px;padding:12px 13px;}" +
    "#blunder-modal .bm-upsell-h{font-family:Antonio,sans-serif;font-weight:700;font-size:15px;color:#8a6d00;margin-bottom:4px;}" +
    "#blunder-modal .bm-upsell-p{font-size:11.5px;line-height:1.4;color:#6b5a2a;margin-bottom:11px;}" +
    "#blunder-modal .bm-go{display:block;width:100%;border:none;border-radius:8px;background:#F5C518;color:#2A2118;font-family:Antonio,sans-serif;font-weight:700;font-size:15px;letter-spacing:1px;padding:10px;cursor:pointer;}" +
    "#blunder-modal .bm-go:hover{background:#ffd633;}" +
    "#blunder-modal .bm-timer{height:3px;background:#C0392B;border-radius:0 0 15px 15px;animation:bmCount 15s linear forwards;}" +
    "@keyframes bmInSide{from{opacity:0;transform:translateY(-50%) translateX(24px);}to{opacity:1;transform:translateY(-50%) translateX(0);}}" +
    "@keyframes bmFade{from{opacity:0;}to{opacity:1;}}" +
    "@keyframes bmCount{from{width:100%;}to{width:0%;}}" +
    "@media(max-width:860px){#blunder-modal{right:50%;top:auto;bottom:14px;transform:translateX(50%);width:min(360px,92vw);max-height:52vh;animation:bmInUp .4s ease;}@keyframes bmInUp{from{opacity:0;transform:translateX(50%) translateY(16px);}to{opacity:1;transform:translateX(50%) translateY(0);}}}";
  document.head.appendChild(st);
}

function showBlunderModal(detail, betterItems, isPremium, square) {
  blunderModalStyle();
  const old = document.getElementById('blunder-modal-bg'); if (old) old.remove();

  let why;
  if (detail && detail.positional) {
    why = 'That move handed over about <strong>' + detail.netLoss + '</strong> pawns of advantage \u2014 exactly the kind of slip the engine pounces on.';
  } else if (detail) {
    why = 'Your <strong>' + detail.lostPiece + '</strong> on <strong>' + square + '</strong> was left hanging \u2014 the ' + detail.attackerPiece + ' from ' + detail.attackerFrom + ' could just take it.';
    if (detail.defended) why += ' Even recapturing, you come out <strong>' + detail.netLoss + '</strong> behind.';
    else why += ' And nothing was guarding it.';
  } else {
    why = 'That move gave up too much material.';
  }

  let bottom;
  if (isPremium && betterItems && betterItems.length) {
    const rows = betterItems.map(function (it) {
      return '<div class="bm-move"><span class="bm-badge" style="background:' + it.color + '">' + it.rank + '</span>' +
        '<div><div><span class="bm-san">' + it.san + '</span><span class="bm-lead">' + it.lead + '</span></div>' +
        '<div class="bm-why">' + it.why + '</div></div></div>';
    }).join('');
    bottom = '<div class="bm-sub">\u2705 What would\u2019ve worked</div>' + rows;
  } else {
    bottom = '<div class="bm-upsell"><div class="bm-upsell-h">\uD83D\uDD13 See the 3 best moves</div>' +
      '<div class="bm-upsell-p">Premium players get the top 3 engine moves \u2014 drawn right on the board \u2014 after every blunder, so you learn exactly what to play instead.</div>' +
      '<button class="bm-go" id="bm-go">Go Premium</button></div>';
  }

  const bg = document.createElement('div'); bg.id = 'blunder-modal-bg';
  bg.innerHTML =
    '<div id="blunder-modal"><button class="bm-x" id="bm-x" aria-label="Close">\u00d7</button>' +
    '<div class="bm-pad"><div class="bm-head">Blunder!</div>' +
    '<div class="bm-body">' + why + '</div>' + bottom + '</div>' +
    '<div class="bm-timer"></div></div>';
  document.body.appendChild(bg);

  return new Promise(function (resolve) {
    let done = false; let to = null;
    const close = function () {
      if (done) return; done = true;
      if (to) clearTimeout(to);
      bg.classList.add('bm-out');
      setTimeout(function () { if (bg.parentElement) bg.remove(); resolve(); }, 400);
    };
    bg.querySelector('#bm-x').onclick = close;
    const go = bg.querySelector('#bm-go');
    if (go) go.onclick = function () { close(); try { showPremium(); } catch (e) {} };
    to = setTimeout(close, 15000);
  });
}

async function blunderReplay(fenBefore, badFrom, badTo, detail) {
  const isPremium = !!(S.user && S.user.is_premium);
  let betterItems = null;
  ...
  if (isPremium) {
    ...
    if (top && top.length) {
      ...
      betterItems = top.map(function (mv, i) {
    ...
    if (detail && detail.attackerFrom && detail.attackerFrom !== '?') {
      const threat = document.querySelector('#board [data-sq="' + detail.attackerFrom + '"]');
      if (threat) threat.classList.add('teach-threat');
    }
  }

  await showBlunderModal(detail, betterItems, isPremium, badTo);

  clearTeachArrows();
  hideTeachPanel();
  document.querySelectorAll('#board .teach-hung, #board .teach-threat').forEach(function (el) {
...
function stopLocalTimer() {
  clearInterval(S.localTimerInterval);
  S.localTimerInterval = null;
  // Freeze the bar at its current width instantly - the CSS `transition: width 1s`
  // otherwise keeps it sliding for up to a second after you move.
  const _tb = document.getElementById('th-fill');
  if (_tb) { _tb.style.transition = 'none'; _tb.style.width = getComputedStyle(_tb).width; }
...
function cbStyle() {
  ...
  st.textContent =
    ...
    "#cb-blurb{font-size:17.5px;line-height:1.6;color:#C4C8D0;text-align:center;margin-bottom:10px;}#cb-blurb strong{color:#F5C518;}" +
    "#cb-hint{font-size:15px;line-height:1.45;color:#7BC86C;text-align:center;min-height:40px;font-family:'JetBrains Mono',monospace;margin-bottom:12px;}#cb-hint strong{color:#F5F1EA;}" +
    ...
    ".cb-text-blurb{font-size:20px;line-height:1.7;color:#C4C8D0;text-align:center;padding:14px 6px 24px;}.cb-text-blurb strong{color:#F5C518;}";
  document.head.appendChild(st);
}
...
function tutorialStyle() {
  ...
  st.textContent =
    ...
    "#tut-body strong{color:#F5C518;}" +
    "#tut-dots{display:flex;gap:7px;justify-content:center;margin-bottom:18px;}" +
    ".tut-dot{width:8px;height:8px;border-radius:50%;background:#3A3F47;transition:background .25s,width .25s;}" +
    ".tut-dot.on{background:#E8722A;width:22px;border-radius:5px;}" +
...
const TUT_STEPS = [
  { icon: '\u265E', title: 'Sudden Death Chess', body: 'Normal chess rules \u2014 with one twist: <strong>a single blunder ends the game instantly.</strong> Leave a piece open to capture, or hand your opponent a winning position, and it\u2019s game over on the spot.' },
...
function showTutorial() {
  tutorialStyle();
  if (document.getElementById('tut-bg')) return;
  let i = 0;
  const bg = document.createElement('div'); bg.id = 'tut-bg';
  const card = document.createElement('div'); card.id = 'tut-card';
  bg.appendChild(card);
  document.body.appendChild(bg);

  const finish = function () {
    try { localStorage.setItem('sdc_tutorial_seen', '1'); } catch (e) {}
    bg.classList.add('tut-out');
    setTimeout(function () { if (bg.parentElement) bg.remove(); }, 350);
  };

  const render = function () {
    const s = TUT_STEPS[i];
    const dots = TUT_STEPS.map(function (_, k) { return '<span class="tut-dot' + (k === i ? ' on' : '') + '"></span>'; }).join('');
    const last = i === TUT_STEPS.length - 1;
    card.innerHTML =
      '<div id="tut-icon">' + s.icon + '</div>' +
...
/* ════════════════════════════════════════════════════════════════════════
   ...
   Features: survival puzzle mode, retry-the-
   blunder, beginner blunder-protection, rating-high / win-streak celebrations,
   first-match coachmark, Enter-to-rematch, new-opponent.
   ════════════════════════════════════════════════════════════════════════ */
(function () {
  'use strict';
  ...
  function ensureStyle() {
    ...
    var css =
      ...
      '.sdx-sq.l{background:#F2E6C9;} .sdx-sq.d{background:#9c6b43;}' +
      ...
      '.sdx-sq img,.sdx-sq svg{width:86%;height:86%;pointer-events:none;filter:drop-shadow(0 0 1.5px rgba(0,0,0,.7));}' +
  ...
  // lobby extras (survival / guard removed)
  ...
  window.SDCX = {
    openRetry: openRetry,
    afterGameOver: afterGameOver, noteBlunder: noteBlunder,
    wireLobby: wireLobby, wireModal: wireModal, showRetryButton: showRetryButton,
    refreshLobbyExtras: refreshLobbyExtras
  };
